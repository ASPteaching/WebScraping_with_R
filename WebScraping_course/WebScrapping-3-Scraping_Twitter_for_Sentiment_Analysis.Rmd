...
- In order to be able to recover tweets "massively" we need to be granted access. This is called "authentication".
- The following objects from the "keys and access token" tab in the developers account are provided after an application has been created
...
1. Extract tweets (continued)
===============================

- We start with a simple search on a "popular" topic

```{r eval=FALSE}
require(twitteR)
search.string <- "#Brexit"
no.of.tweets <- 1000
aTopicTweets <- searchTwitter(search.string, n=no.of.tweets, lang="en",
                          since='2019-04-01', until='2019-04-10')
head(aTopicTweets)
save(aTopicTweets, file="aTopicTweets.Rda")
```

Alternatively we can search (at most) recent tweets from a given user.
...
if (!exists("aTopicTweets")) load("aTopicTweets.Rda")
 ...
 tweets.text<-  gsub( '[^[:alnum:]_]', ' ', tweets.text)
...
2. Clean the text (2a): Remove "common words"
=========================================
...
2. Clean the text (2b): Code for removing `stopwords`
==============================
...
- alternatively, if we were in an election time we could compare tweets about parties.
