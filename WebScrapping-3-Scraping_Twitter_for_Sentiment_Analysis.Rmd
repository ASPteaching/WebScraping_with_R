...
- Social networks like Twitter provide an excellent example for web scraping:
...
- For the purpose of this exercise, we will need the following packages:
...
- In order to connect with Twitter from a Windows system we need to download a  a bunch of certificates that allow the system to confirm that the site where you are connecting is where you relly intend to connect.
...
- Recently the TwitteR package has changed its acreditation system, moving from using the `ROAuth` package to using the `httr` package.
- These means that some instructions found in the web that make use of this approach may not work.
- We describe below what is called "alternative" authentication.
- For this method you need the following objects from the "keys and access token" tab in your developers account
...
key <- NULL         # "your API key from twitter"
secret <-  NULL  # "your Secret key from twitter"
secrettk <-  NULL   # "Access Token Secret from Twitter"
mytoken <-  NULL    # "Access Token from Twitter"
```
...
- Type `? searchTwitter` to know more about this function.
